#!/bin/bash
# Launchd-safe wrapper for the daily Amex sync. Mirrors run-rogers.sh.
#
...
cd "$REPO"
echo "===== budget-sync amex @ $(date) ====="
# NOTE: runs HEADED (no --headless). Amex blocks automated/headless logins; a
# headed run in the trust-built profile passes. A browser window appears for
...
for attempt in $(seq 1 "$ATTEMPTS"); do
  ...
  status=0
  "$NODE" "$TSX" "$REPO/sync/run-amex.ts" || status=$?
  if [ "$status" -eq 0 ]; then
    echo "✓ amex sync succeeded on attempt $attempt @ $(date)"
    echo "ok" > "$STATUS_DIR/amex"
  ...
  echo "✗ amex sync failed on attempt $attempt (exit $status) @ $(date)"
...
echo "✗ amex sync gave up after $ATTEMPTS attempts @ $(date)"
