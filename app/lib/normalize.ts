...
export function masterCategoryFor(description: string | null): string | null {
  ...
  if (has('dental')) return 'Dental'
  if (has('drug store', 'pharmacies', 'medical', 'health')) return 'Health'
  // Anything car-related — fuel, parking, automotive service — is "Cars".
  if (has('fuel', 'service station', 'gas')) return 'Cars'
  if (has('parking')) return 'Cars'
  if (has('automotive', 'car and truck', 'vehicle')) return 'Cars'
  // Public transit only stays in Transport.
  if (has('commuter', 'transportation', 'ferries')) return 'Transport'
  if (has('software', 'digital goods')) return 'Subscriptions'
  if (has('telecommunication')) return 'Subscriptions'
  if (has('utilities', 'cable')) return 'Utilities'
  if (has('furniture', 'home furnishings', 'hardware')) return 'Shopping'
